...
public class TP1021 {
    ...
    public static void hitungKebahagiaanMaksimalO1(int x, int[] souvenirPrice, int[] souvenirHappiness) {
        ...
        for (int i = 1; i <= length; i++) {
            for (int j = 0; j <= totalBudget; j++) {
                if (souvenirPrice[i - 1] <= j) {
                    if (i >= 3) {
                        tempSum[i][j] = Math.max(tempSum[i - 1][j], souvenirHappiness[i - 1] + tempSum[i - 3][j - souvenirPrice[i - 1]]);
                    } else if (i == 2) {
                        tempSum[i][j] = Math.max(tempSum[i - 1][j], souvenirHappiness[i - 1] + tempSum[i - 2][j - souvenirPrice[i - 1]]);
                    } else {
                        tempSum[i][j] = Math.max(tempSum[i - 1][j], souvenirHappiness[i - 1]);
                    }
                } else {
        ...
        out.println(tempSum[length][totalBudget]);
    }
